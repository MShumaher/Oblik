...
namespace Oblik.Domain.Repositories.EntityFramework
{
    public class EFPatientsRepository : IPatientsRepository
    {
        ...
        public Patient GetPatientByCodeWord(string codeWord)
        {
            return context.Patients.FirstOrDefault(x => x.CodeWord == codeWord);
        }

        public Patient GetPatientByTelNumb(int TelNumb)
        {
            return context.Patients.FirstOrDefault(x => x.TelNumb == TelNumb);
        }
